...
const TravelPlan = ({ toggleIsEdit, cityLat, cityLong }: TravelPlanProps) => {
	...
	useEffect(() => {
		if (plan && plan.dayOrder) {
			dayRefs.current = plan?.dayOrder.map(
				(_, index) => dayRefs.current[index] || React.createRef()
			)
		}
	}, [plan])
	...
	useEffect(() => {
		const checkDayPositions = () => {
			...
			const headerBottom =
				headerRef.current?.getBoundingClientRect().bottom ?? 424
			...
			dayRefs.current.forEach((ref, index) => {
				if (ref.current) {
					...
					if (
						(dayTop >= headerTop - buffer && dayTop <= headerBottom + buffer) ||
						(dayBottom >= headerTop - buffer &&
							dayBottom <= headerBottom + buffer) ||
				...
				<div className="pl-4 pr-6 h-10 flex justify-between items-center bg-white">
					<div>
						<span className="text-sm font-semibold mr-2">
							day {visibleDay ? visibleDay : currentDay}
						</span>
						<select
