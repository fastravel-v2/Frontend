...
export const getTravelDetail = async (id: string): Promise<ITravelDetail> => {
	// api
	const travelDetail: { data: IRTravel } = await tokenInstance.get(
		`core/travel/plan/${id}`
	)
	// const travelDetail: {data: IRTravel} = await tokenInstance.get(`travel/plan/${id}`)

	const dayOrder = generateDayOrder(
		travelDetail.data.info.info.startDate,
		travelDetail.data.info.info.endDate
	)
...
export const getNearbyLocations = async (
	...
	const nearbyLocations: { data: IPlaceInfo[] } = await tokenInstance.post(
