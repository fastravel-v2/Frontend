...
export const useUrlStore = create<UrlStore>((set) => ({
    ...
    fetchUrlsForRepository: async (repositoryId) => {
        ...
        set({ urls: urls.map((entry) => ({ url: entry.url, checked: false } as UrlItem)) });
    },
    toggleCheck: (index) => {
        set((state) => ({
    ...
    addUrlGlobal: (repositoryId: string, url: string) => {
        set((state) => {
            const updatedUrls = state.urls.map((item) =>
                item.repositoryId === repositoryId ? { ...item, url } : item
            );
			console.log(repositoryId, url)
			
			//디버~그
			const urlsForRepository = updatedUrls.filter((item) => item.repositoryId === repositoryId);
			console.log(`URLs for repositoryId ${repositoryId}:`, urlsForRepository);
