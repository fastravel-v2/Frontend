    ...
    return (
        ...
        >
            {(provided: DraggableProvided, snapshot: DraggableStateSnapshot) => (
                ...
                >
                    <div className={`absolute left-[26px] top-6 h-5 w-5 rounded-full flex justify-center items-center ${isSelected ? 'bg-green3' : 'bg-white border border-darkGray3'} `}
                        onClick={handleSelect}
                    >
                        <IoMdCheckmark className={`${isSelected ? 'text-white' : 'text-darkGray3'}`} />
                    </div>
